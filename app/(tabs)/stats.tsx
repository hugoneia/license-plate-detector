...
const [exclusionZonesConfig, setExclusionZonesConfig] =
...
// Tipos que deben aparecer en Estadísticas según el catálogo central.
const statisticParkingTypes = useMemo(
() => PARKING_TYPE_LIST.filter((type) => type.showInStatistics),
[]
);
...
// Memoizar cálculos costosos basados en visibleEntries.
const grouped = useMemo(
() => groupLicensePlates(visibleEntries),
[visibleEntries]
);

const uniqueStats = useMemo(
() => getUniquePlateStats(visibleEntries),
[visibleEntries]
);

const topPlates = useMemo(
() => getTopPlatesByDetections(visibleEntries, 5),
[visibleEntries]
);

const allByCount = useMemo(
() => grouped.sort((a, b) => b.count - a.count),
[grouped]
);

// Estadísticas totales sin filtros.
const totalStatsNoFilter = useMemo(
() => getUniquePlateStats(rawEntries),
[rawEntries]
);
...
const isAnyFilterActive = exclusionZonesConfig.masterEnabled;

const excludedDetections = hasActiveFilters
? totalStatsNoFilter.totalDetections -
(uniqueStats?.totalDetections || 0)
: 0;

const excludedUnique = hasActiveFilters
? totalStatsNoFilter.totalUnique -
...
← Volver
      ...
      >
        {selectedPlate.entries.map((item, index) => {
          ...
          const hasGPS =
            item.location && item.location !== "NO GPS";
            ...
            >
              <View className="flex-row items-center justify-between mb-2">
                <Text className="font-semibold text-foreground">
                  Detección #
                  {selectedPlate.entries.length - index}
                </Text>
              </View>
...
Cargando estadísticas...
    ...
    <Text className="text-sm text-muted mt-2">
      Analizando {rawEntries.length} registros
    </Text>
  </ScreenContainer>
);
...
Estadísticas
    ...
    <Text className="text-base text-muted mt-1 mb-4">
  ...
  >
    <View className="gap-6 p-6">
      ...
      {uniqueStats.totalDetections > 0 ? (
        <View className="bg-surface rounded-2xl border border-border flex-row">
          <View className="flex-1 p-4 items-center justify-center">
            ...
            {isAnyFilterActive && (
              <Text
                className={`text-sm text-error font-semibold mt-1 ${
        ...
        <View className="bg-surface rounded-2xl p-6 border border-border items-center">
          <Text className="text-muted text-center">
            No hay matrículas detectadas todavía. ¡Comienza
            a capturar matrículas para ver estadísticas!
          </Text>
        </View>
      )}
      ...
      {uniqueStats.totalDetections > 0 &&
        parkingTypeStats.length > 0 && (
          <View className="bg-surface rounded-2xl border border-border">
            {parkingTypeStats.map(
              ({ type, visible, excluded }, index) => (
                <View key={type.id}>
                  {index > 0 && (
                    <View
                      style={{
          ...
          uniqueStats.totalDetections > 0 && (
            <View className="gap-2">
              {topPlates.map((plate, index) => {
                let positionColor = colors.primary;
                let positionBgColor =
                  "rgba(0, 102, 204, 0.1)";

                if (index === 0) {
                  positionColor = colors.warning;
                  positionBgColor = `${colors.warning}15`;
                } else if (index === 1) {
                  positionColor = colors.muted;
                  positionBgColor = `${colors.muted}15`;
                } else if (index === 2) {
                  positionColor = "#D88A2D";
                  positionBgColor = "#D88A2D15";
                }
                  ...
                  >
                    <View className="flex-1">
                      ...
                      <Text className="text-xs text-muted ml-11">
                        {plate.count} detecciones • Última:{" "}
                        {new Date(
                          plate.entries[0]?.timestamp ||
                            Date.now()
