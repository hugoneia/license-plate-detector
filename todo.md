...
## Funcionalidades de Exportación, GPS y Borrado Total (Pendientes)

- [x] **Exportación CSV no bloqueante**: Diferido el procesamiento pesado 50 ms tras activar `isExporting`, manteniendo manejo de errores y restauración del estado de carga.
- [x] **GPS en Entrada Rápida del Historial**: Integrado `useGeolocation`, capturada ubicación al abrir el modal y aplicada una última lectura antes de guardar; `NO GPS` queda solo como fallback.
- [x] **Borrado Total Seguro**: Añadido `deleteAllPlates` al contexto y un botón rojo en Ajustes con advertencia y cuenta atrás de 5 segundos antes de confirmar.
- [x] **Validación final de las nuevas funcionalidades**: Tests y TypeScript superados; el borrado total conserva el backup y pasa por la capa centralizada de persistencia.
